...
local mouse = { } do
    ...
    UserInputService.InputBegan:Connect(function(inputObject)
        if inputObject.UserInputType == Enum.UserInputType.MouseButton1 or inputObject.UserInputType == Enum.UserInputType.Touch then
            mouse.held = true
            mouse.InputBegan:Fire(inputObject)  -- 传递 inputObject 以便获取触摸位置
        end
    end)
...
local function getMouse()
    -- GetMouseLocation 在触摸设备上也能返回最后一个触摸位置
...
local dragger = {} do
    ...
    function dragger.new(frame)
        ...
        -- 用于检测是否在标题栏区域（顶部 22 像素）
        local function isInTitleBar(inputPosition)
            local absPos = frame.AbsolutePosition
            local absSize = frame.AbsoluteSize
            -- 标题栏高度固定为 22（与 frame 初始高度一致）
            return inputPosition.Y >= absPos.Y and inputPosition.Y <= absPos.Y + 22
        end

        -- 鼠标/触摸进入事件（可选，用于高亮，但不影响拖动判断）
        ...
        mouse.InputBegan:Connect(function(inputObject)
            -- 只有当按下位置在标题栏内且没有其他交互（颜色选取、滑动）且是顶层窗口时才启动拖动
            if findBrowsingTopMost() == frame then
                if (not colorpicking) and (not sliding) then
                    -- 获取按下时的位置（从 inputObject 或 getMouse）
                    local pressPos
                    if inputObject and inputObject.UserInputType == Enum.UserInputType.Touch then
                        pressPos = inputObject.Position
                    else
                        pressPos = getMouse()
                    end
                    if isInTitleBar(pressPos) then
                        local objectPosition = Vector2.new(pressPos.X - frame.AbsolutePosition.X, pressPos.Y - frame.AbsolutePosition.Y)
                        while mouse.held do
                            pcall(function()
                                local currentMouse = getMouse()
                                resize(frame, { Position = UDim2.new(0, currentMouse.X - objectPosition.X + (frame.Size.X.Offset * frame.AnchorPoint.X), 0, currentMouse.Y - objectPosition.Y + (frame.Size.Y.Offset * frame.AnchorPoint.Y)) }, 0.1)
                            end)
        ...
        function self.new(tabOptions)
            ...
            local types = { } do
                ...
                function types.color(colorOptions)
                    ...
                    dragger.new(colorPicker)  -- 同样应用改进后的拖拽器
                ...
                function types.dropdown(dropdownOptions)
                    ...
                    dragger.new(dropdownWindow)  -- 同样应用改进后的拖拽器
